...
class BookDetail(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        # Prevent seeing other users' books by guessing pk
...
# -------------------- Tags --------------------
# Fix: use FormMixin + ListView (instead of ListView+CreateView)
class TagListCreate(LoginRequiredMixin, FormMixin, ListView):
    model = Tag
    ...
    def get_context_data(self, **kwargs):
        ...
        # ensure a form is always in context (GET and invalid POST)
        if 'form' not in context:
            context['form'] = self.get_form()
        return context

    def post(self, request, *args, **kwargs):
        # FormMixin doesn't rely on self.object; just process the form
        form = self.get_form()
        if form.is_valid():
            form.save()
            return redirect(self.get_success_url())
        # On errors, re-render list + invalid form
